...
def main():
    ...
    for epoch in range(epochs):
        ...
        for step, data in enumerate(train_bar):     # 遍历训练集,
            ...
            #acc_train += torch.eq(output, labels.to(device)).sum().item()
            err1, err5 = accuracy(output.data, labels, topk=(1, 5))
            acc_train += 1-err1
        ...
        with torch.no_grad():
            ...
            for val_data in val_bar:
                ...
                err1, err5 = accuracy(outputs.data, val_labels, topk=(1, 5))
                acc += 1 - err1
